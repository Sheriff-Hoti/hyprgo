...
type KittyGridOpts struct {
	Cols        uint32 // display width in terminal columns
	Rows        uint32 // display height in terminal rows
	RowsSpacing uint32 // spacing between rows in terminal rows
	ColsSpacing uint32 // spacing between columns in terminal columns
	TopSpacing  uint32 // spacing at top and bottom in terminal rows
	LeftSpacing uint32 // spacing at left and right in terminal columns
	ImgWidth    uint32 // image width in pixels
	ImgHeight   uint32 // image height in pixels
}
...
// func KittyWriteFiles(out io.Writer, absfileNames []string, grid KittyGridOpts) ([]Cell, error) {
// 	result := make([]Cell, 0, len(absfileNames))
// 	fnames_idx := 0
// 	if grid.Cols <= 0 {
...
// 			KittyWriteFile(out, absfileNames[fnames_idx], KittyImgOpts{
...
// 				Filename: absfileNames[fnames_idx],
